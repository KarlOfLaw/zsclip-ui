...
impl MainUiLayout {
    ...
    pub(crate) fn fit_to_client(mut self, client_w: i32, client_h: i32) -> Self {
        ...
        self.win_w = client_w.max(side_margin * 2 + 1);
        self.seg_w = (self.win_w - self.seg_x - side_margin).max(1);
        self.list_w = (self.win_w - self.list_x - side_margin).max(1);
        self.list_h = (client_h - self.list_y - bottom_margin).max(self.row_h + self.list_pad * 2);

        let first_title_button_x = self.win_w - 4 - self.btn_w * 4 - self.btn_gap * 3;
        self.search_w = (first_title_button_x - 6 - self.search_left).max(80);
        self
    }
        ...
        if input.visible_len > 0 {
            ...
            for index in start_idx..end_idx {
                ...
                if let Some(background) = background {
                    ...
                    row_background_commands.push(MainPaintCommand::RoundFill {
                        rect: rect.inflate(-2, -1),
                        fill: MainPaintFill::Theme(fill),
                        radius: 6,
                    });
                    if background == MainRowBackground::Selected {
                        row_background_commands.push(MainPaintCommand::RoundFill {
                            rect: UiRect::new(
                                rect.left + 2,
                                rect.top + 9,
...
mod tests {
    ...
    fn main_render_plan_describes_visible_regions_without_host_renderer() {
        ...
        assert_eq!(plan.row_background_commands.len(), 5);
        assert!(matches!(
        ...
        assert!(matches!(
            plan.row_background_commands[2],
            MainPaintCommand::RoundFill {
                fill: MainPaintFill::Theme(MainThemeRole::ItemHovered),
                radius: 6,
                ..
    ...
    #[test]
    fn main_layout_fits_resizable_client_without_changing_density_metrics() {
        let base = MainUiLayout::zsclip();
        let fitted = base.fit_to_client(480, 720);

        assert_eq!(fitted.win_w, 480);
        assert_eq!(fitted.segment_rect().right, 472);
        assert_eq!(fitted.list_rect().right, 472);
        assert_eq!(fitted.list_rect().bottom, 713);
        assert_eq!(fitted.row_h, base.row_h);
