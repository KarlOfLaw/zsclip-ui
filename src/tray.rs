...
use crate::app::state::{apply_shared_tab_view_state, AppSettings, WindowRole};
...
unsafe fn position_window_at_restore(
    hwnd: HWND,
    state: &AppState,
    restore_x: i32,
    restore_y: i32,
...
fn resolve_main_window_position(
    ...
    let (win_w, win_h) =
        restored_size.unwrap_or_else(|| main_window_size_for_point(anchor_point));
...
unsafe fn position_main_window_for_state(hwnd: HWND, state: &AppState, by_hotkey: bool) {
    ...
    if state.settings.edge_auto_hide {
        if let Some((restore_x, restore_y)) = edge_preferred_position(state) {
            position_window_at_restore(hwnd, state, restore_x, restore_y);
            return;
        }
    ...
    let (x, y, win_w, win_h) = resolve_main_window_position(
        &state.settings,
        by_hotkey,
        pt,
        state.role,
