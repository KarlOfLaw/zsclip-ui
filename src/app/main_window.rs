...
pub(super) unsafe fn handle_main_window_size(hwnd: HWND, size: UiSize, minimized: bool) {
    let ptr = get_state_ptr(hwnd);
    if !ptr.is_null() {
        let state = &mut *ptr;
        if minimized {
...
pub(super) unsafe fn handle_main_theme_changed(hwnd: HWND) {
    let ptr = get_state_ptr(hwnd);
    if !ptr.is_null() {
