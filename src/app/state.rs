...
pub(crate) struct AppSettings {
    ...
    pub(crate) image_zoom_preview_enabled: bool,
    ...
    pub(crate) last_window_w: i32,
    pub(crate) last_window_h: i32,
    pub(crate) last_quick_window_w: i32,
    pub(crate) last_quick_window_h: i32,
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            image_zoom_preview_enabled: true,
            ...
            last_window_w: 0,
            last_window_h: 0,
            last_quick_window_w: 0,
            last_quick_window_h: 0,
            sticker_x: -1,
            sticker_y: -1,
...
impl AppState {
    pub(super) fn new(
        ...
        let ui_dpi = unsafe { crate::platform::dpi::layout_dpi_for_window(hwnd) };
        let base_layout = main_layout_for_dpi(ui_dpi);
        let client_size = main_window_client_bounds(hwnd)
            .map(|bounds| UiSize {
                width: bounds.width(),
                height: bounds.height(),
            })
            .unwrap_or(UiSize {
                width: base_layout.win_w,
                height: base_layout.list_y + base_layout.list_h + 7,
            });
