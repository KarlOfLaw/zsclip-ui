...
fn windows_main_window_appearance_uses_main_window_host() {
    ...
    let theme_start = main_window
        .find("pub(super) unsafe fn handle_main_theme_changed")
        .unwrap();
    let theme_end = main_window[theme_start..]
        .find("
pub(super) unsafe fn handle_main_window_moved")
        .map(|offset| theme_start + offset)
        .unwrap();
    let theme_block = &main_window[theme_start..theme_end];
    ...
    assert!(theme_block.contains("apply_main_window_appearance(hwnd)"));
    ...
    assert!(!theme_block.contains("platform_appearance::set_rounded_corners(hwnd)"));
    assert!(!theme_block.contains("platform_appearance::apply_dark_mode_to_window(hwnd)"));
